...
@Client.on_message((["ریلود",  f"reload@{BOT_USERNAME}"]) & other_filters)
...
async def update_admin(client, message):
    ...
    await message.reply_text(
        "✅ **لیست ادمین **باموفقیت  ** اپدیت شد**"
    )


@Client.on_message(command(["بعدی", f"skip@{BOT_USERNAME}", "vskip"]) & other_filters)
@authorized_users_only
async def skip(client, m: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    text="• ◄°l||l°   منو °l||l°►", callback_data="cbmenu"
                ),
                InlineKeyboardButton(
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            await m.reply("❌ هیچ چیزی در حال حاضر پخش نمی شود")
        elif op == 1:
            await m.reply("✅ __صف ها__ **a .خالی است .**\n\n**• کاربر ربات چت صوتی را ترک می کند**")
        elif op == 2:
            await m.reply("🗑️ **پاک کردن صف ها**\n\n**• کاربر ربات چت صوتی را ترک می کند**")
        else:
            await m.reply_photo(
                photo=f"{IMG_3}",
                caption=f"⏭ **اهنگ بعدش پخش شد.**\n\n🏷 **اسم:** [{op[0]}]({op[1]})\n💭 **گپ:** `{chat_id}`\n💡 **وضعیت:** `A+ درحال پخش`\n🎧 **درخواست شده توسط:** {m.from_user.mention()}",
